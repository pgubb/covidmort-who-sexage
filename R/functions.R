...
get_raw_data <- function(url = "https://cdn.who.int/media/docs/default-source/world-health-data-platform/covid-19-excessmortality/2022-03-25_covid-19_gem.zip?sfvrsn=2d5835e0_2", 
                         sheet = "Country by year, sex and age") {
  ...
  target_file <- unzip(temp, files[[1]][1]) # 
  sheets <- excel_sheets(target_file)
  
  read_excel(target_file, sheet = sheet, skip = 11) 
...
get_data_types <- function(clean_data) {
  ...
  datatype_bysex <- read_csv("./data/who_data_bysex.csv") %>% select(iso3c, Deaths_2020, Deaths_2021)
...
simulate <- function(analysis_data, sel_iso, sel_source, sel_year, n = 1000) { 
  ...
  df_p <- do.call("rbind", replicate(n, df_p, simplify = FALSE)) %>% mutate(sim = rep(1:n, each = 10))
...
obs_mr <- analysis_data %>% 
  ...
  filter(source_year != "All cause deaths (expected): 2021") %>% 
  rename(value = mr) %>% 
  ...
  df %>%
    ...
    mutate(
      term_label = case_when(
        term == "(Intercept)" ~ "1. Baseline mortality rate per 100,000 (Females)", 
        term == "male" ~ "2. Male:Female mortality (rate ratio)", 
        term == "Age_Lower_rec" ~ "3. Age-slope of mortality (rate ratio)", 
        term == "male:Age_Lower_rec" ~ "4. Marginal effect of male on age-slope (rate ratio) ",
      ), 
      term_label2 = case_when(
        term == "(Intercept)" ~ "1. Baseline mortality rate per 100,000 (Females)", 
        term == "male" ~ "2. Male:Female mortality (rate ratio)", 
        term == "Age_Lower_rec" ~ "3. Age-slope of mortality for females (rate ratio)", 
        term == "male:Age_Lower_rec" ~ "4. Age-slope of mortality for males (rate ratio) ",
      ), 
