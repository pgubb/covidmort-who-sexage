...
plot_obs <- function(observations, slct_metric = "Relative risk (Males/Females)") {
  ...
  ggplot(data_to_plot,
    ...
    # Scales
    scale_color_manual(values = c( "All cause deaths (expected): 2020" = colors[2], "Excess deaths: 2020" = colors[1])) +

    # Plot labels
    labs(
      title = paste0(unique(data_to_plot$metric), " in mortality"),
      subtitle = "Based on unmodified data from WHO",
      caption = "Source: Author's calculations with WHO excess mortality dataset",
      y = NULL,
      x = "Age (Lower bound)"
    ) +
...
visualize_coeffs <- function(model_results, models = NULL, coefs = NULL) {
  ...
  model_results %>%
    group_by(source, Year, model_no, model_label, term_short, term_label2) %>%
    summarize(
  ...
  data_to_plot <- bind_rows(model_results, average)
  ...
  ggplot(data = data_to_plot,
         aes(y = fct_reorder(Country, gni_pcap_ppp),
             x = str_wrap(term_label2, 12),
             fill = fillvalue)) +
    ...
    labs(
      title =  "Estimated coefficients from poisson model",
      #subtitle = subtitle,
      y = NULL,
      x = NULL,
      caption = "*Averages are population-weighted.") +
